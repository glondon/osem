class PopulateData < ActiveRecord::Migration
  def up
    # Create users
    user_admin = User.new(email: 'admin@email.com', password: Devise.friendly_token[0,20], name: 'Admin')
    user_admin.skip_confirmation!
    user_admin.save!

    10.times do |i|
      user = User.new(email: "user#{i}@email.com", password: Devise.friendly_token[0,20], name: "User#{i}")
